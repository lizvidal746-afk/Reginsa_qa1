...
Este archivo fue movido a [docs/GUIA_REPORTES.md](docs/GUIA_REPORTES.md).
...
- name: Publish Allure report
  uses: simplygoodwork/github-action-allure-report@master
  with:
...
1. **Limpia reportes viejos**: `rm -rf playwright-report allure-results`
2. **Ejecuta con video**: Agrega `video: 'on'` en `playwright.config.js`
3. **Ejecuta con traces**: Ya está en config: `trace: 'on-first-retry'`
4. **Exporta resultados**: Los JSON en `allure-results/` son portables
